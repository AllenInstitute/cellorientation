...
def load(
    loc="data_files", blocksize=1024, anndata_write=True, anndata_name="mouse_retina"
):
    ...
    # if anndata doesn't exit alread, download inputs and construct it
        ...
        for fname, url in files.items():
            if not os.path.exists(os.path.join(loc, fname)):
                download_file(loc=loc, blocksize=blocksize)

        # read in data
        df_obs = pd.read_csv(
            "data/10x_mouse_retina_development_phenotype.csv", index_col=0
        )[["barcode", "sample", "age", "CellType"]]
        df_var = pd.read_csv(
            "data/10x_mouse_retina_development_feature.csv", index_col=0
        )[["id", "gene_short_name"]]
